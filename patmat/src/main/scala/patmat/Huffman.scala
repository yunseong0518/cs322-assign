...
object Huffman {
    ...
    def decodeAcc(treeAcc: CodeTree, bits: List[Bit], acc: List[Char]): List[Char] = {
      treeAcc match {
        ...
        case Fork(left, right, chars, _) => {
          bits match {
            case 0::tail => decodeAcc(left, tail, acc)
            case 1::tail => decodeAcc(right, tail, acc)
          }
        }
      }
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encodeAcc(tree: CodeTree, acc: List[Bit])(text: Char): List[Bit] = {
      tree match {
        case Fork(left, right, chars, weight) => {
          encodeAcc(left, acc:::(0::Nil))(text):::encodeAcc(right, acc:::(1::Nil))(text)
        }
        case Leaf(char, _) => {
          if (text == char) acc
          else Nil
    ...
    text.map(x => encodeAcc(tree, Nil)(x).head)
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertAcc(treeAcc: CodeTree, acc: List[Bit]): CodeTable = {
      treeAcc match {
        case Leaf(char, _) => List((char, acc))
        case Fork(left, right, chars, _) => {
          convertAcc(left, acc:::(0::Nil)):::convertAcc(right, acc:::(1::Nil))
        }
      }
    }
    convertAcc(tree, Nil)
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    text.map(x => codeBits(codeTable)(x)).flatten
  }
}
